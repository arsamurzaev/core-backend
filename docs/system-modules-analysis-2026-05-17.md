...
| `attribute` | Пользовательские атрибуты | Есть repository/service/tests | Связь с product/product-type может разрастись | Наружу дать schema/value contracts |
| `audit` | Аудит изменений | Небольшой infra-модуль | Может стать случайной свалкой логов | Сделать единый audit sink через events |
| `auth` | Сессии, guards, login, handoff | Центральный infrastructure module | Его импортируют многие controllers | Выделить публичные decorators/guards как sanctioned infra boundary |
| `brand` | Бренды каталога | Простой CRUD | Связь с product через DTO/filters | Держать как справочник, наружу только brand summary |
| `capability` | Возможности каталога | Уже есть ports/contracts | Правила могут дублироваться на frontend/backend | Сделать единый effective capability contract и тесты |
...
| `catalog-sale-unit` | Единицы продаж | Отдельный справочник | Может быть смешан с price variants | Связать через sellable/sale-unit contract |
...
| `cron` | Планировщики | Оркестратор | Импортирует concrete modules | Перевести на ports и domain commands |
...
| `inventory` | Склады, остатки, резервы, движения | Есть reservation/stock ports | Нужно строго держать variantId как ключ | Все операции только через variantId и inventory policy |
| `metric` | Метрики | Небольшой модуль | Дублирование observability | Четко отделить business metrics от technical observability |
| `observability` | HTTP observability, metrics/tracing | Хороший infra-модуль | Может стать зависимостью domain-кода | Давать только logger/metrics interfaces |
...
| `s3` | Storage/media upload | Infrastructure module | Domain-модули зависят от S3 напрямую | Переименовать/обернуть как MediaStoragePort |
| `seo` | SEO сущности и sync | Отдельный модуль | Product/Catalog знают repository | Перевести синхронизацию на domain event handlers |
...
- `integration/contracts.ts`: `ORDER_EXPORT_PORT`, `CATALOG_SYNC_PORT`, `STOCK_SYNC_PORT`, provider registry.
- `inventory/contracts.ts`: `INVENTORY_RESERVATION_PORT`, `INVENTORY_STOCK_READER_PORT`, `INVENTORY_MOVEMENT_PORT`, `INVENTORY_MODE_PORT`.
- `product/contracts.ts`: `PRODUCT_COMMAND_PORT`, `PRODUCT_READER_PORT`, `PRODUCT_SNAPSHOT_PORT`, `PRODUCT_VARIANT_RESOLVER_PORT`, `PRODUCT_PRICING_PORT`, `PRODUCT_SELLABLE_READER_PORT`.
- `product-type/contracts.ts`: schema/variant attribute contracts.
...
- `cron -> inventory/product/catalog`: планировщик импортирует concrete modules.
...
- Добавить backend architecture boundary test по аналогии с frontend.
- Запретить cross-module imports внутренних файлов.
- Разрешить только `contracts.ts`, `public.ts`, shared infra.
- Заполнить `public.ts` у всех core-модулей.
